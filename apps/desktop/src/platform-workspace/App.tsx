      ...
      if (action === 'rename') {
        ...
        try {
          ...
          setModal({
            type: 'confirm-detected-name',
            account,
            detectedName: detection.accountName,
  ...
  return (
    <div className="h-screen w-full overflow-hidden bg-slate-50 text-slate-900">
      <header className="relative z-20 flex h-[72px] items-center border-b border-slate-200 bg-white px-4 shadow-sm">
              ...
              return (
                <div key={account.id} className="relative flex shrink-0 items-center">
                  <button
                    ...
                    title={account.alias}
                  >
                    <span className={`h-2 w-2 shrink-0 rounded-full ${statusColors[account.runtimeStatus]}`} />
        ...
        {error && (
          <div className="absolute right-4 top-[58px] z-50 flex max-w-md items-center gap-2 rounded-md border border-rose-200 bg-white px-3 py-2 text-xs font-medium text-rose-700 shadow-lg">
            <span className="truncate">{error}</span>
            <button type="button" onClick={() => setError('')} aria-label="关闭提示" title="关闭">
              <X size={14} />
            </button>
          </div>
        )}
        ...
        <dl className="space-y-4 text-sm">
          ...
          <div className="border-t border-slate-200 pt-4">
            ...
            <dd className="mt-1 text-xs text-slate-400">
              来源：{source === 'document_title' ? '客服接待页面标题' : '客服接待页面内容'}
            </dd>
          </div>
        </dl>
        <div className="mt-6 flex justify-end gap-2">
