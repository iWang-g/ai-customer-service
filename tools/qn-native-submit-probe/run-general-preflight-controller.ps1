...
$resultFile = Join-Path $repoRoot '.tmp\qn-native-submit-probe\general-preflight-result.log'
$taskName = "CodexQnGeneralPreflight-$PID"
$taskEntry = Join-Path $probeRoot 'run-general-preflight-task.ps1'
...
try {
  ...
  if (-not (Test-Path -LiteralPath $resultFile)) { throw 'interactive preflight timed out' }
