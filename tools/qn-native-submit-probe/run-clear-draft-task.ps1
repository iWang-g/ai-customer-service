...
$outFile = Join-Path $runDir 'clear-draft.log'
...
try {
  ...
  if (-not $request.requireMinimized) {
    throw 'Draft cleanup requires requireMinimized=true.'
  }
  ...
  if ($empty.value.isEmpty) {
    throw 'Input box is already empty; refusing draft cleanup.'
  }
