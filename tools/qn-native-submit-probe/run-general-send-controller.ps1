...
$taskName = "CodexQnGeneralSend-$PID"
$taskEntry = Join-Path $probeRoot 'run-general-send-task.ps1'
...
try {
  ...
  if (-not (Test-Path -LiteralPath $resultFile)) { throw 'interactive send timed out' }
