...
class QaCategoryTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        init_db()
        self.base = create_base(KnowledgeBaseCreate(name="售前问答库", kind="qa"))
    ...
    def test_default_and_custom_categories_are_persisted(self) -> None:
        defaults = list_qa_categories(self.base["id"])
        ...
        custom = create_qa_category(self.base["id"], QaCategoryCreate(name="会员问题"))
        self.assertFalse(custom["is_builtin"])
        self.assertEqual(list_qa_categories(self.base["id"])[-1]["name"], "会员问题")

        with self.assertRaises(HTTPException) as context:
            create_qa_category(self.base["id"], QaCategoryCreate(name="会员问题"))
        self.assertEqual(context.exception.status_code, 409)

    def test_entries_support_category_filter_keyword_search_and_pagination(self) -> None:
        categories = {item["name"]: item for item in list_qa_categories(self.base["id"])}
        logistics = categories["物流问题"]
        common = categories["常见问题"]

        create_qa_entry(self.base["id"], QaEntryCreate(
        ...
        logistics_page = list_qa_entries(self.base["id"], logistics["id"])
        self.assertEqual(logistics_page["total"], 1)
        self.assertEqual(logistics_page["items"][0]["category"], "物流问题")

        uncategorized_page = list_qa_entries(self.base["id"], "uncategorized")
        self.assertEqual(uncategorized_page["total"], 1)
        self.assertEqual(uncategorized_page["items"][0]["category_id"], "")

        search_page = list_qa_entries(self.base["id"], keyword="营业时间")
        self.assertEqual(search_page["total"], 1)
        self.assertEqual(search_page["items"][0]["answer"], "全天在线。")

        first_page = list_qa_entries(self.base["id"], page=1, page_size=1)
        second_page = list_qa_entries(self.base["id"], page=2, page_size=1)
        ...
        counted = {item["name"]: item["item_count"] for item in list_qa_categories(self.base["id"])}
