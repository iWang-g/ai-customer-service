"""Douyin consultation, clarification and human-operation boundaries."""
import re

CLARIFICATION_PROMPT = (
    '抖店须区分意思不明确与问题明确但缺少依据。返回布尔needs_clarification：'
    ...
    '抖店不执行邮件看图等千牛专属流程，无法提供客户明确要求的服务时转人工。'
    '未知卡片格式、收到图片本身不是转人工理由。douyin_unknown_message是未核验的消息核心JSON，'
    '结合其正文、最近对话和正式商品/订单/知识资料理解需求；字段和客户声明不等于平台核验事实。'
    '卡片里的申请退款、修改地址可能只是按钮或功能标题，不代表客户提出办理请求；不因关键词就转接。'
    'douyin_unread_image或[图片]表示只知道客户发了图片，本系统没有识图能力，不能声称看懂图片、描述画面或识别图中文字。'
...
ANSWER_PROMPT = (CLARIFICATION_PROMPT +
    ...
    '订单只覆盖本次首屏，顺序和总数未知，不默认第一项是目标订单，空结果不等于没有下单。'
...
def explicit_human_operation(message: str) -> bool:
    """Narrow affirmative requests only; contextual/ambiguous cases stay with the model."""
    for clause in re.split(r'[，,。！？!?；;\n]|但是|不过', message or ''):
        if re.search(r'不(?:用|要|需要|想)|无需|别|只是问|先问|如果|假如|比如|例如|[“”「」]|了解|咨询|规则|政策|流程|怎么|如何|能不能|可以吗|支持|是否', clause):
            continue
        if re.search(r'^(?:请|麻烦)?(?:帮我)?(?:转人工|转客服|找人工|找客服|人工客服)[吧呀啊呢\s]*$', clause.strip()):
            return True
        if re.search(r'(?:帮我|给我|我要|我想|我需要|请).{0,6}(?:申请退款|申请退货|办理退款|办理退货|退货退款|退款|退货|换货|修改地址|改地址|安排定制|定制)|按这张图.{0,4}(?:做|制作|定制)', clause):
            return True
    return False
...
def knowledge_query(message: str, platform_context: list) -> str:
    ...
    latest = next((item for item in reversed(platform_context) if isinstance(item, dict)
                   and item.get('type') in {'douyin_unknown_message', 'douyin_unread_image'}), {})
    if latest.get('type') != 'douyin_unknown_message':
        return message
    ...
    for field in fields[:32]:
        ...
        if (isinstance(path, str) and re.search(r'(?:^|\.)(?:content|hintContent|title|description|desc|product_name|goods_name|reason)$', path)
