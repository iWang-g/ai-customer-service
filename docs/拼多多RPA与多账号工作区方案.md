...
- Cookie
- LocalStorage 原始内容
- 未经用户授权或未加密的拼多多密码；用户主动配置的密码仅通过本机系统安全存储保存，不同步云端
- 验证码
- 完整浏览器 Profile
...
- 用户手动完成扫码登录、验证码和风控验证。
- 经用户在可信工作区弹窗明确授权后，可使用 Electron `safeStorage` 在本机加密保存拼多多账号密码，用于凭证过期后的单次自动登录；未登录页面保存凭据后再次确认才启动本次登录，应用重启恢复时采用固定保守等待和逐字填写。验证码、账号异常、盗号风险等状态立即停止，不采用随机化伪装或任何风控绕过；禁止静默捕获、明文存储或上传云端。
- 工作区窗口非模态且单实例。
- Cookie 和缓存只保存在本机。
- 第三方网页使用最小权限 preload。
